...
class EntraLoginHandler(BaseHandler, tornado.auth.OAuth2Mixin):
    _OAUTH_NO_CALLBACKS = False
    _OAUTH_SETTINGS_KEY = 'oauth'

    @property
    def base_url(self):
        return f"https://login.microsoftonline.com/{self.settings[self._OAUTH_SETTINGS_KEY]['tenant_id']}"

    @property
    def _OAUTH_AUTHORIZE_URL(self):
        return f"{self.base_url}/oauth2/v2.0/authorize"
    ...
    @property
    def _OAUTH_USER_INFO_URL(self):
        return "https://graph.microsoft.com/v1.0/me?$select=mail"
    ...
    async def _on_auth(self, access_token_response):
        ...
        decoded_body = json.loads(response.body.decode('utf-8'))
        email = (decoded_body.get('mail') or '').strip()
        email_verified = (
            decoded_body.get('mail') and
            authenticate(self.application.options.auth, email)
        )
